...
import { staggerContainer, staggerChild, viewportMid } from "@/branding/motion";
...
export function HowHadeWorks() {
  return (
    ...
    >
      ...
      >
        {steps.map((step, index) => (
          <Fragment key={step.number}>
            ...
            >
              <p className="font-mono text-[11px] font-semibold tracking-[0.14em] text-accent">
                {step.number}
              </p>
      ...
      <motion.div
        className="hidden md:grid md:grid-cols-[1fr_1px_1fr_1px_1fr]"
        variants={staggerContainer}
        initial="hidden"
        whileInView="visible"
        viewport={viewportMid}
        ...
        {steps.map((step, index) => (
          <Fragment key={step.number}>
            <motion.article
              ...
              className={`flex flex-col bg-white/[.92] p-6 backdrop-blur ${
